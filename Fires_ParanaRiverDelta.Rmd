...
date: "8/7/2020, updated 3/22/2021"
output:
...
## Input the spatial objects to R

### Data source
- Paraná River Delta limits: shapefile of the study area. Save this vector in the path _/data/study_area/_
- Hotspots from FIRMS-NASA, https://firms.modaps.eosdis.nasa.gov/map/#t:adv;d:2020-07-28..2020-08-03;@-59.9,-33.1,8z . Save the downloaded zip files in the path _/data/zip/_
...
### Naming conventions of the FIRMS data: 
  ...
  - DL_FIRE_J1.xx if you requested VIIRS 375m data from NOAA-20 (JPSS-1). _For quick analyses, I usually don't request these data because hotspots may be redundant with VIIRS S-NPP._

### Spatial data pre-processing

1. Read files and unzip
2. Handle the files as spatial data
3. Merge
4. Re-project
5. Clip by study area 
...
The code is prepared to detect the appropriate strings and generate the spatial objects, based on the naming conventions of FIRMS-NASA.

```{r read_spatial_data, eval=T, include = T}
library(sf)
library(stringr)
...
Now we need to reproject the hotspot data to the same CRS than the study area.
...
Lastly, we clip the hotspots by the study area.
```{r clip, eval=T, include =T}
...
Optional: write the hotspot layer.
```{r save_shp, eval=F, include =T}
sf::st_write(hotspots_all, "output/hotspot_all_2020.gpkg")

```

### _Importing a pre-processed hotspot layer as spatial object_

Alternatively, the script can also begin with a pre-processed final hotspot layer.

```{r read_file, eval = F, include = TRUE}
library(sf)
...
Optional: write the VIIRS hotspot layer.
```{r save_shp, eval=F, include =T}
...
We may save these plots.
```{r historical_save, eval=T, include=T}
ggsave("output/Focos_historicos_ambossensores.png", plot = MODIS_VIIRS_plot, width = 9, height = 5, dpi = 300)
ggsave("output/Focos_historicos_ambossensores_english.png", plot = MODIS_VIIRS_plot_eng, width = 9, height = 5, dpi = 300)
```
