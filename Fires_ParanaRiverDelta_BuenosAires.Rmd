...
library(sf)
hotspots_all <- st_read("data/hotspots/focos_BsAs_20200117.shp", quiet=TRUE) 
...
plot_diario <- focos_cum %>%
  ...
  geom_col() +
  scale_color_manual(values=c("#990000", "#990000"),  labels = c("Focos diarios", "Nuevos focos")) +
  xlab("Fecha") +
  ylab("Cantidad de focos de calor") +
  scale_x_date(date_labels = "%d/%m/%Y", breaks = "week") +
  scale_y_continuous(breaks=seq(from = 0, to = 2000, by = 10)) +
  theme_bw()+
  ...
  labs(title="Incendios en el Delta del Paraná de Buenos Aires (Argentina) durante 2020", subtitle = "Focos de calor por día, en base a datos VIIRS de FIRMS-NASA", caption = "Natalia Morandeira; 3iA-UNSAM")  
...
ggsave("output/Focos_diario_BuenosAires.png", plot = plot_diario, width = 8, height = 5, dpi = 300)
...
plot_acum <- focos_cum %>%
  ...
  labs(title="Incendios en el Delta del Paraná (Argentina) durante 2020", subtitle = "Focos de calor por día y acumulados al 4/10/2020, en base a datos VIIRS de FIRMS-NASA", caption = "Natalia Morandeira; 3iA-UNSAM")  
...
plot_cum_eng <- focos_cum %>%
  ...
  labs(title="Potential fires at the Paraná River Delta (Argentina)", subtitle = "Daily hotspots and cummulative numbers up to 10/04/2020, based on VIIRS data - FIRMS-NASA", caption = "Natalia Morandeira; 3iA-UNSAM")  
